...
def personSimilarity(u,v):
    top_part = 0
    bottom_part_a = 0
    bottom_part_b = 0
    for col in range(0,len(matrix_str[0])):
        top_part = top_part + ((float(matrix_str[u][col]) - averages[u]) * (float(matrix_str[v][col]) - averages[v]))
        bottom_part_a = bottom_part_a + ((float(matrix_str[u][col]) - averages[u]) * (float(matrix_str[u][col]) - averages[u]))
        bottom_part_b = bottom_part_b + ((float(matrix_str[v][col]) - averages[v]) * (float(matrix_str[v][col]) - averages[v]))
...
def prediction(row,col,topk):
    first = getFirst(topk,row)
    second = getSecond(topk,row,first[0][0])
    
    a = first[0][0] * float(matrix_str[first[0][1]][col])
    b = second[0][0] * float(matrix_str[second[0][1]][col])
    result = float(a + b) / (float(matrix_str[first[0][1]][col]) + float(matrix_str[second[0][1]][col]) +1 ) 
...
def getTopk():
    ...
    topk_id = []
    temp_topk = []
    for u in range(0, len(matrix_str)):
        for v in range(0, len(matrix_str)):
            temp_topk.append([personSimilarity(u,v),v])
